...
export default function BoutiqueCreditsPage() {
  ...
  // Modal de Règlement d'Ardoise / Crédit
  ...
  const handleWhatsAppRelance = (fac: Facture) => {
    ...
    const message = `Bonjour ${clientNom}, ${etabNom} vous rappelle poliment qu'après votre acompte de ${avance} FCFA sur la commande #${facNum} (Total: ${total} FCFA), il reste un solde dû de ${manquant} FCFA. Merci de passer en boutique ou nous contacter pour régler. À très bientôt !`;
    ...
    <div className="min-h-screen bg-[#FBF7EF] text-[#1B4332] flex flex-col lg:flex-row font-sans">
      ...
      <main className="flex-1 lg:ml-64 p-4 lg:p-8 space-y-6">
        ...
        <div className="pb-4 border-b border-[#E2D5C3] flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          <div>
            <span className="text-xs font-black uppercase tracking-widest text-[#B8442C] bg-[#B8442C]/10 px-2.5 py-0.5 rounded-full border border-[#B8442C]/30">
              Gestion des Créances Clients Boutique
            </span>
            <h1 className="font-serif text-2xl lg:text-3xl font-black text-[#1B4332] mt-1 flex items-center gap-2">
              ...
              Argent à Récupérer (Crédits Boutique)
            </h1>
            <p className="text-xs text-gray-600 font-medium mt-0.5">
              Suivez les montants dus sur les achats d'articles et effectuez vos relances directes sur WhatsApp.
            </p>
          </div>
        ...
        <div className="p-6 rounded-3xl bg-[#B8442C] text-white shadow-xl flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
          <div>
            <span className="text-xs font-bold uppercase tracking-widest opacity-80">Total Créances Clients Boutique</span>
            ...
            <p className="text-xs opacity-90 font-medium mt-1">
              {facturesCredit.length} commande(s) avec solde restant dû
            </p>
          </div>
        </div>

        {/* Liste des Créances Boutique */}
        <div className="space-y-3">
          {filteredCredits.length === 0 ? (
            <div className="p-8 text-center bg-[#F3ECE0] rounded-3xl border-2 border-dashed border-[#E2D5C3] space-y-2">
              <CheckCircle2 className="w-10 h-10 text-emerald-600 mx-auto" />
              <p className="font-serif font-bold text-base text-[#1B4332]">Aucun crédit client en cours</p>
              <p className="text-xs text-gray-500">Toutes les ventes en boutique sont entièrement réglées !</p>
            </div>
          ) : (
          ...
          <div className="fixed inset-0 z-50 bg-black/80 backdrop-blur-md flex items-center justify-center p-4">
            <form onSubmit={handlePayCredit} className="bg-[#F3ECE0] border-2 border-[#E2D5C3] rounded-3xl p-6 w-full max-w-md shadow-2xl space-y-4">
              <div className="flex items-center justify-between pb-2 border-b border-[#E2D5C3]">
                <h3 className="font-serif font-black text-xl text-[#1B4332]">Enregistrer un Règlement</h3>
