...
import { Etablissement, Utilisateur, Produit, MouvementStock, Facture, Caisse } from '@/types';

export default function SnackDashboardPage() {
  ...
  const loadData = () => {
    try {
      ...
      const listCaisses = offlineDB.getCaisses();
  ...
  const isServeuseOrCaissiere = ['Serveuse', 'Caissière', 'Employé'].includes(currentUser?.role || '');
  ...
  const handleAddMouvementRapide = (e: React.FormEvent) => {
    ...
    offlineDB.addMouvementStock({
      ...
      note_motif: motifInput || `Ajustement rapide snack ${typeMvt}`,
    });
  ...
  const handleDirectWhatsAppRelance = (fac: Facture) => {
    ...
    const message = `Bonjour ${clientNom}, ${etabNom} vous rappelle votre ardoise de ${manquant} FCFA sur la commande #${facNum} (Total: ${total} FCFA). Merci et bon appétit !`;
  ...
  return (
    <AppLayout>
        ...
        {!isServeuseOrCaissiere && isTrialExpired && (
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-[#E2D5C3]">
          <div>
            ...
            <p className="text-xs text-[#1B4332]/70 font-medium">
              Prise de commande express, gestion des serveuses et clôture de caisses du Snack.
            </p>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span>Prise de Commande & Caisses ➔</span>
            </Link>
          </div>
        </div>

        {/* Grid KPIs Spécifiques Snack */}
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <div className="p-4 rounded-3xl bg-white border border-[#E2D5C3] shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Chiffre d'Affaires 7j</p>
          ...
          <div className="p-4 rounded-3xl bg-[#1B4332] text-white rounded-3xl p-4 shadow-sm flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-[#E8A33D] text-[#0F291E] flex items-center justify-center text-xl font-bold">
              🍟
            </div>
            <div>
              <p className="text-[11px] font-bold text-[#E8A33D] uppercase tracking-wider">Caisses Ouvertes</p>
              <p className="font-serif font-black text-xl text-white">
                {caisses.filter((c) => c.active).length} <span className="text-xs font-bold text-gray-300">caisse(s)</span>
              </p>
            </div>
          </div>

          <div className="p-4 rounded-3xl bg-white border border-[#E2D5C3] shadow-sm flex items-center gap-4">
            <div className="w-12 h-12 rounded-2xl bg-orange-100 text-orange-800 flex items-center justify-center text-xl font-bold">
              🍗
            </div>
            <div>
              <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Plats & Boissons en Stock</p>
              <p className="font-serif font-black text-xl text-[#1B4332]">
                {produits.reduce((acc, p) => acc + (p.quantite_totale || 0), 0)} <span className="text-xs font-bold text-gray-500">portions</span>
              </p>
            </div>
          </div>
          ...
          <div className="p-4 rounded-3xl bg-white border border-[#E2D5C3] shadow-sm flex items-center gap-4">
            ...
            <div>
              <p className="text-[11px] font-bold text-gray-500 uppercase tracking-wider">Ardoises / Crédits</p>
              <p className="font-serif font-black text-xl text-[#1B4332]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          {/* Formulaire Mouvement Rapide Stock Snack */}
          <div className="p-5 rounded-3xl bg-white border border-[#E2D5C3] shadow-sm space-y-4">
            <div className="flex items-center gap-2">
              <Package className="w-5 h-5 text-[#B8442C]" />
              <h2 className="font-serif font-black text-lg text-[#1B4332]">Entrée / Sortie Ingrédients & Plats</h2>
            </div>
            ...
            <form onSubmit={handleAddMouvementRapide} className="space-y-3">
              <div>
                <label className="block text-[11px] font-bold uppercase text-gray-600 mb-1">Plat / Boisson</label>
                <select
                ...
                >
                  {produits.map((p) => (
                    <option key={p.id} value={p.id}>
                      {p.nom} (Stock actuel: {p.quantite_totale} portions)
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <select
                    ...
                    className="w-full p-2.5 text-xs rounded-xl bg-[#FBF7EF] border border-[#E2D5C3] font-bold text-[#1B4332]"
                  >
                    <option value="entree">➕ Entrée Stock / Cuisson</option>
                    <option value="sortie">➖ Vente Directe</option>
                    <option value="casse_perte">⚠️ Perte / Ingrédient Périmé</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[11px] font-bold uppercase text-gray-600 mb-1">Quantité (Portions)</label>
                  <input
                    type="number"
                    min="1"
                    value={quantiteInput}
              ...
              <div>
                ...
                <input
                  type="text"
                  placeholder="ex: Arrivage poulet, frites pré-cuites..."
                  value={motifInput}
          ...
          {/* Alertes Stock Faible Plats */}
          <div className="p-5 rounded-3xl bg-white border border-[#E2D5C3] shadow-sm space-y-4 lg:col-span-2">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <AlertTriangle className="w-5 h-5 text-amber-600" />
                <h2 className="font-serif font-black text-lg text-[#1B4332]">Plats & Ingrédients à Seuil Critique ({lowStockProduits.length})</h2>
              </div>
              <Link href="/snack/produits" className="text-xs font-bold text-[#B8442C] underline">
                Voir tout le stock ➔
              </Link>
            </div>
            ...
            {lowStockProduits.length === 0 ? (
              <div className="p-8 text-center bg-[#FBF7EF] rounded-2xl border border-dashed border-[#E2D5C3]">
                ...
                <p className="font-bold text-xs text-[#1B4332]">Aucune rupture de stock au Snack.</p>
              </div>
            ) : (
                  ...
                  <div key={p.id} className="p-3 rounded-2xl bg-[#FBF7EF] border border-amber-200 flex items-center justify-between text-xs">
                    ...
                    <div className="text-right">
                      <span className="font-black text-amber-800 bg-amber-100 px-2.5 py-1 rounded-full text-[11px]">
                        Reste : {p.quantite_totale} (seuil: {p.seuil_alerte})
                      </span>
                    </div>
                  </div>
                ))}
        ...
        {/* Relances WhatsApp Crédits Snack */}
        {facturesCredit.length > 0 && (
          <div className="p-5 rounded-3xl bg-white border border-[#E2D5C3] shadow-sm space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Send className="w-5 h-5 text-emerald-600" />
                <h2 className="font-serif font-black text-lg text-[#1B4332]">Relances Crédits Clients Snack (WhatsApp Direct)</h2>
              </div>
              <Link href="/snack/credits" className="text-xs font-bold text-[#B8442C] underline">
              ...
              {facturesCredit.slice(0, 6).map((fac) => (
                <div key={fac.id} className="p-4 rounded-2xl bg-[#FBF7EF] border border-[#E2D5C3] space-y-2">
                  ...
                  <p className="text-[11px] text-gray-600">
                    Commande #{fac.numero_facture} • Total : {fac.montant_total.toLocaleString('fr-FR')} FCFA
                  </p>
                  <button
